...
async function readJsonFiles(root, subdir) {
    const out = [];
    try {
...
export default function Export({ root, navigateHome }) {
    const [matchUrl, setMatchUrl] = useState(null);
    const [pitUrl, setPitUrl] = useState(null);
    const [error, setError] = useState(null);
    async function exportMatches() {
        setError(null);
        if (!root)
            return setError('Pick a data folder first');
        try {
            const files = await readJsonFiles(root, 'matches');
            if (!files.length)
                return setError('No matches to export');
    ...
    async function exportPit() {
        ...
        try {
            ...
            if (!files.length)
                return setError('No pit data to export');
            // CSV header and rows
