...
import { ArrowLeft, User, Hash, Users } from 'lucide-react';
...
export const GameStart: React.FC<GameStartProps> = ({ matchData, setMatchData, setView }) => {
  ...
  useEffect(() => {
    ...
    // Auto-fill initials if not already set or if empty
  ...
  return (
    <div className="flex flex-col h-full w-full p-6 md:p-12 items-center justify-center">
      <div className="w-full max-w-4xl bg-white/90 dark:bg-obsidian-light/80 backdrop-blur-xl border border-slate-200 dark:border-slate-700 rounded-3xl p-8 shadow-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <Input 
                        label="Initials" 
                        type="text" 
                        value={matchData.scouterInitials}
                        onChange={(e) => updateField('scouterInitials', e.target.value)}
                        placeholder="XYZ"
            ...
            <div className="space-y-6 flex flex-col justify-center">
                 <div className="space-y-2 relative">
                    ...
                    {/* Schedule Suggestions - Horizontal Scroll */}
                    {suggestedTeamsFromMatch.length > 0 && (
                        <div className="flex gap-2 mb-4 overflow-x-auto pb-2 no-scrollbar">
                            {suggestedTeamsFromMatch.map(t => (
                                <button
                                    key={t}
                                    onClick={() => selectTeam(t.toString())}
                                    className={`flex-none px-6 py-3 rounded-lg font-bold transition-all ${matchData.teamNumber === t.toString() ? 'bg-indigo-600 text-white ring-2 ring-indigo-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'}`}
                                >
                                    {t}
                                </button>
                            ))}
                    ...
                    <Input 
                        ...
                        autoComplete="off"
                        className="text-4xl font-black h-20 pl-6"
                    />
                    ...
                    {suggestions.length > 0 && (
                        <div className="absolute z-50 w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-b-xl shadow-2xl max-h-48 overflow-y-auto mt-1">
                            {suggestions.map(team => (
                                <div 
                                    key={team.teamNumber}
                                    className="px-6 py-4 hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer text-slate-900 dark:text-white border-b border-slate-100 dark:border-slate-700/50 last:border-0 flex justify-between items-center"
                                    onClick={() => selectTeam(team.teamNumber.toString())}
                                >
                ...
                <div className="pt-4">
                    <Button 
                        ...
                        fullWidth 
                        disabled={!isValid}
                        onClick={() => setView('AUTO_START')}
                        className="h-16 text-xl shadow-xl shadow-matcha/20"
                    >
                        START PRE-MATCH
                    </Button>
                </div>
