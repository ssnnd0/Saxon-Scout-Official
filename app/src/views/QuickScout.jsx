 ...
 */
export default function QuickScout({ root, scouter }) {
    ...
    function toggleEndgame() {
        setStats(prev => {
            const newStats = structuredClone(prev);
            const cycle = ['none', 'park', 'shallow', 'deep'];
    ...
    function clickCell(r, c) {
        if (r === 0 && c === 0)
            return recordCoral('L1');
        if (r === 0 && c === 1)
            return recordCoral('L2');
        if (r === 0 && c === 2)
            return recordCoral('L3');
        if (r === 1 && c === 0)
            return recordCoral('L4');
        if (r === 1 && c === 1)
            return recordAlgae('processor');
        if (r === 1 && c === 2)
            return recordAlgae('net');
        if (r === 2 && c === 0)
            return toggleEndgame();
        if (r === 2 && c === 1)
            return recordFoul();
        if (r === 2 && c === 2)
            return undo();
    ...
    async function save() {
        ...
        const sumCoral = (phase) => Object.values(stats[phase].coral).reduce((a, b) => a + b, 0);
        const sumAlgae = (phase) => Object.values(stats[phase].algae).reduce((a, b) => a + b, 0);
        const record = {
            team,
            game,
            alliance,
    ...
    return (<div class="card shadow-sm">
      <div class="card-body">
        <div class="d-flex flex-wrap gap-2 mb-3 align-items-center">
          ...
          <input class="form-control w-auto" type="number" placeholder="Team" value={team || ''} onInput={(e) => setTeam(parseInt(e.target.value || '0'))}/>
          <input class="form-control w-auto" type="number" placeholder="Match" value={game || ''} onInput={(e) => setGame(parseInt(e.target.value || '0'))}/>
          {/* Save button */}
          <button class="btn btn-success ms-auto" onClick={save}>Save Match</button>
        </div>
        {/* Render the grid of buttons */}
        <div class="row g-2">
          {Array.from({ length: GRID.rows }).map((_, r) => (<div class="col-12 d-flex gap-2" style={{ height: '7rem' }}>
              {Array.from({ length: GRID.cols }).map((_, c) => (<button key={`${r}-${c}`} class="flex-fill btn border" style={sliceStyle(r, c)} onClick={() => clickCell(r, c)}>
                  .
                </button>))}
            </div>))}
        </div>
